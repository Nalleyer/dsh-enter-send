...
- 换行动作不再依赖 `document.execCommand`（在当前 Lexical composer 上实测失效），改为合成 Shift+Enter keydown 走 composer 原生换行路径；若上游调整 Enter / Shift+Enter 的 keymap 处理，需同步更新 `newlineViaComposer`（见 `src/client/keymap.ts`）。
